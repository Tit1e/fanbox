<!--
[INPUT]: 依赖 package.json 构建脚本、electron/main.js 系统集成和 server.js 平台分支
...
> 初稿写于 2026-06-18，2026-07 按当前代码修订。FanBox 的主力验证平台仍是 macOS。
...
- `server.js` 的命令探测仍通过 `/bin/zsh -lc 'command -v …'`，另有 `/bin/sh` 和 `du` 调用；这些需要统一的平台适配层。
...
Codex 在 Windows 的安装、凭据和会话行为由上游决定。应先用目标版本做最小真机探针；如果 CLI 自身不稳定，FanBox 只能如实降级，不能在应用层掩盖。
...
- **Windows**：优先使用系统电源 API；合盖动作由电源计划控制，FanBox 不应擅自永久修改用户设置。
...
2. 为 `server.js` 收敛命令探测、磁盘统计、代理和凭据的平台适配层。
